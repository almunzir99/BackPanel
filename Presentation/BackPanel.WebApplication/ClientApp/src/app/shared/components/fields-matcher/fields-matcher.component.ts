...
export class FieldsMatcherComponent implements OnInit {
  ...
  onSubmit() {
    ...
    var shortestArrayLength = this.toItems.length > this.fromItems.length ? this.fromItems.length : this.toItems.length;
    for (let index = 0; index < shortestArrayLength; index++) {
      map.set(this.fromItems[index], this.toItems[index]);
    }
