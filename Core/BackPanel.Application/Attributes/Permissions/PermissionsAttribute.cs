...
public class PermissionAttribute : Attribute, IAsyncActionFilter
{
    ...
    private static async Task<Permission?> GetPermission(ActionExecutingContext context)
    {
        ...
        // Resolve Mediator and get role by title
        var mediator = context.HttpContext.RequestServices.GetService<Mediator>();
        var role = await mediator!.Send(new GetByTitleRoleQuery(title));
        if (role != null)
        {
